...
void Log::Init(HMODULE hModule) {
	...
	if (GetModuleFileNameA(hModule, g_logFile, MAX_PATH) != 0) {
		...
		sprintf_s(chLogBuff, szTimestamp, "Joker v1");
		if (slash != -1) {
			g_logFile[slash + 1] = '\0';
			strcpy_s(g_debugLogFile, g_logFile);
			strcat_s(g_debugLogFile, "Joker.log");
...
void Log::Pattern(const char* fmt, ...) {
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
...
void Log::Banner(const char* fmt, ...) {
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleTextAttribute(hConsole, 2);
	std::cout << fmt << "\n";
}
